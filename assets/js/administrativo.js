(function() {
    ...
    const colunasPresencial = ['Sala 1', 'Sala 2', 'Sala 3', 'Sala 4', 'Sala 5', 'Grupo'];

    function createDropdownOptions() {
        // Popula o dropdown com os usernames dos profissionais
    ...
    async function init() {
        try {
            // Busca os profissionais do Cloud Firestore, não do Realtime Database
            ...
            // Configura o listener em tempo real para as grades a partir do Firestore
            const gradesDocRef = db.collection('administrativo').doc('grades');
            gradesDocRef.onSnapshot((doc) => {
                dadosDasGrades = doc.exists ? doc.data() : {};
                
                const activeMainTabEl = document.querySelector('#main-tabs button.active');
                const activeDayTabEl = document.querySelector('#app-content .tab-nav button.active');
            ...
            // Renderiza a primeira visualização
    ...
    async function autoSaveChange(selectElement) {
        ...
        const hora = horaCell.textContent.replace(":", "-"); // Firestore não aceita ':' em nomes de campo
        const colIndex = selectElement.closest('td').cellIndex - (row.querySelector('.period-cell') ? 2 : 1);
        const newValue = selectElement.value;
        
        const activeMainTab = document.querySelector('#main-tabs button.active').dataset.tab;
        const activeDayTab = document.querySelector('#app-content .tab-nav button.active').dataset.day;
        
        // Caminho de campo para o Firestore (ex: online.segunda.08-00.col0)
        ...
        try {
            const gradesDocRef = db.collection('administrativo').doc('grades');
            // Usa update para salvar apenas o campo modificado

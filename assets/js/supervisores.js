    ...
    auth.onAuthStateChanged(async user => {
        if (user) {
            try {
                ...
                if (userDoc.exists && (userDoc.data().funcoes?.includes('supervisor') || userDoc.data().funcoes?.includes('admin'))) {
                    renderSupervisorCards();
                } else {
                    if(dashboardContent) dashboardContent.innerHTML = '<h2>Acesso Negado</h2><p>Esta área é exclusiva para supervisores e administradores.</p>';
                }
            } catch (error) {
                 if(dashboardContent) dashboardContent.innerHTML = '<h2>Erro ao verificar permissões.</h2>';
            }
        } else {
